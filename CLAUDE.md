...
**One line each — title + a single clause naming the headline artifact;
no second sentence.** If you find yourself adding rationale, test
counts, sub-decisions, or a “which also…” clause, it belongs in
...
every session’s context; keeping it terse is what makes that affordable.
Add each new milestone’s full detail to `MILESTONES.md` in numeric order
...
- **M31** — Correctness & output-honesty sweep (scaled ESEM fit under
...
This section is the **status slot only** — what is in flight right now,
...
- **In flight:** nothing. Last numbered milestone: **M52**
  (factorability diagnostics). Since then, PR \#50 (2026-07-08,
  between-milestone) shipped the package logo/favicons, the
  version-gated EFA k=1 oracle test, and an EFAtools 0.8.0 compatibility
  fix for
  [`suggest_k()`](https://jmgirard.github.io/ackwards/reference/suggest_k.md)’s
  CD criterion — this is what actually re-opened the post-0.1.0 dev
  cycle, bumping DESCRIPTION `0.1.0` → `0.1.0.9000` (`0.1.0` was the
  CRAN-pinned release; the `.9000` note previously here was
  aspirational). Detail in `MILESTONES.md` (“Between-milestone
  changes”); user-facing notes in `NEWS.md`.
...
- For a milestone: a detailed entry added to `MILESTONES.md` **in
  ...
  milestones” (title + a single clause — see that section’s header).
  `MILESTONES.md` is the single source of truth for milestone history —
  never re-log it in DESIGN.md §15 (a pointer), and never let the
  CLAUDE.md index grow past one line per milestone.
- For a **non-milestone** code change (any merged PR touching
  `R/`/`tests/`/`DESCRIPTION`/vignettes that isn’t a planned, numbered
  milestone — a between-milestone review fix, a hotfix): add a dated
  entry to `MILESTONES.md`’s “Between-milestone changes” section (keeps
  the numbered list gap-free) and a `NEWS.md` line if user-visible.
  Don’t leave code changes logged only in git.
