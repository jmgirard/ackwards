...
osf <- list(
  matrix    = list(guid = "s9bjz", name = "corSpearman_AMH.csv",
                   url = "https://osf.io/download/s9bjz/"),
  functions = list(guid = "7jfkw", name = "ExtendedBassAckwards functions with annotation.R",
                   url = "https://osf.io/download/7jfkw/")
)

tmp <- tempfile(fileext = "_amh"); dir.create(tmp)
...
attr(amh, "provenance") <- list(
  source   = "Forbes, M. K. (2023). Improving hierarchical models of individual differences: An extension of Goldberg's bass-ackward method. Psychological Methods. doi:10.1037/met0000546",
  osf      = "https://osf.io/pcwm8/",
  license  = "CC-BY 4.0 International (https://creativecommons.org/licenses/by/4.0/)",
  files    = vapply(osf, function(f) sprintf("%s (guid %s) <%s>", f$name, f$guid, f$url), character(1)),
  note     = "R = published 155-variable AMH Spearman matrix; comp_corr/cong computed with Forbes's ExtendedBassAckwards reference implementation. Only ackwards() runs at test time.",
