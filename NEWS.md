...
* The default-output baseline test now uses a platform tolerance (1e-8 for
  ...
  BLAS differences. No package output changed.
...
* **Plain-English vignettes, second batch.** The recommended-workflow article,
  the Forbes-extension article, the Forbes (2023) reproduction, the ordinal
  data article, and the interpreting-and-labeling article were revised in the
  same style, which completes the set. Every statistical term is explained in
  plain words where each article first uses it. The code in those articles
  and the results they show did not change. The settings that reproduce
