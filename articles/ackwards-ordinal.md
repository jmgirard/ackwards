...
everything in between. An ordinal item is recorded on an ordered scale
with few categories. Treating these items as continuous and computing
Pearson correlations between them is common in practice. But it
introduces systematic bias that can distort both the loadings and the
between-level edges that bass-ackwards analysis depends on. A loading is
the correlation between an item and a factor, and a factor is an
...
categories, the observed correlation between two items drops. It is
always lower than the correlation between their underlying latent
variables. The coarser the rating scale, the larger the attenuation. For
a 5-point scale, Pearson correlations can be 10 to 20% lower than the
true latent correlations. For a 2-point (binary) scale, the
underestimate can exceed 30%.
...
2.  **Between-level edges are attenuated.** The hierarchy looks flatter.
    Factors that should be nearly perfectly correlated across levels may
    appear only moderately correlated.
3.  **The number of factors is underestimated.** Attenuation compresses
    the eigenvalue spectrum, so selection criteria may suggest fewer
    factors than truly exist. Parallel analysis, which keeps a factor
    only when it explains more variance than random data of the same
    size would, is one such criterion, and MAP is another.
...
Tetrachoric is just the two-category special case of polychoric.
...
`R`, with a polychoric matrix. All downstream computation then operates
on that polychoric `R`: factor extraction, rotation, tenBerge scoring,
and the between-level algebra. A rotation re-orients the factors within
a level without changing how well they fit.

## Automatic detection

[`ackwards()`](https://jmgirard.github.io/ackwards/reference/ackwards.md)
checks for ordinal-looking data using a simple heuristic. If any column
is integer-valued with 7 or fewer distinct levels, the data is flagged,
and a warning is issued when `cor = "pearson"` (the default).
...
correlations among the five neuroticism items (`N1` to `N5`), Pearson
against polychoric.
...
correlations are consistently higher. The `N1` and `N2` pair, for
instance, goes from about 0.73 (Pearson) to 0.79 (polychoric). Across
the block the increase runs to roughly 0.05, enough to shift loadings
and change eigenvalues.

### Loadings

The table below compares primary loadings, the loading of each
Neuroticism item on its dominant factor, under both correlation bases at
k = 5. The Δ column is the attenuation: how much larger each loading
...
Polychoric loadings for the Neuroticism items are noticeably higher. The
latent structure is sharper when the attenuating effect of coarse
categories is removed.

### Between-level edges

The same comparison for the primary-parent edges. The Δ column is the
change in connection strength (\|r\|). It is positive where the
polychoric basis recovers a stronger connection by undoing attenuation,
and negative in the few cases where the recovered structure is slightly
looser. Using absolute values makes the direction read correctly even
for the negatively-signed edge.

[TABLE]

The edges are broadly similar in sign and rank order, so the hierarchy
is the same, but polychoric edges are stronger. Factors that represent
...
weighted least squares, mean- and variance-adjusted). ESEM (exploratory
structural equation modeling) fits the factor model in lavaan and adds
standard errors. WLSMV is the standard estimator for structural equation
models with categorical or ordinal indicators, and it is the same method
Mplus uses by default.

WLSMV genuinely operates on the polychoric basis. The lavaan package
estimates the item thresholds and the polychoric correlations among the
ordinal indicators. It then fits the factor model to *that* matrix using
a diagonal weight matrix. So combining `cor = "polychoric"` with the
ESEM engine is not stacking two separate corrections. The polychoric
structure is precisely what WLSMV is built to model, and the
...
better than Pearson-based fit indices on the same data. That reflects
both the better model specification and the different reference
distribution WLSMV uses.
...
| 5 to 7 ordered categories (typical Likert) | `cor = "polychoric"` (recommended) |
| 3 to 4 categories | `cor = "polychoric"` (strongly recommended) |
...
25-item scale. Computation is usually fast (seconds), but it scales as
...
[`psych::polychoric()`](https://rdrr.io/pkg/psych/man/tetrachor.html)
fail under its default continuity correction (set `correct = 0`). With
many such items it can also drive the correlation matrix
*near-singular*, in which case per-level fit indices (especially CFI,
which comes back `NA`) and factor scores become unreliable. A factor
score is each person’s estimated position on a factor. Run
\[[`check_items()`](https://jmgirard.github.io/ackwards/reference/check_items.md)\]\[ackwards::check_items\]
before fitting to see which items are degenerate or sparse. Then read
the **“When to trust the result”** section of
[`?ackwards`](https://jmgirard.github.io/ackwards/reference/ackwards.md)
...
[`summary()`](https://rdrr.io/r/base/summary.html) keep reminding you.
...
because the KMO bands and `N:p` rules of thumb are contested. But a very
low KMO or a handful of low-MSA items is a useful early signal. The
[`ackwards()`](https://jmgirard.github.io/ackwards/reference/ackwards.md)
function runs a light version of this screen internally. It warns only
at the consequential extreme (KMO below .5, `N:p` below 5, or `k_max`
past the Ledermann bound for EFA/ESEM). EFA (exploratory factor
analysis) models the items as a small number of shared factors plus
item-specific noise.
...
> not precisely 1. That scaling detail is the only reason
