...
correlations only for *adjacent* levels: 1↔︎2, 2↔︎3, 3↔︎4, and so on. A
factor is an unobserved dimension that explains why a set of items
correlate, and a factor score is each person’s estimated position on a
factor. Forbes (2023) extended the method in two ways. Her method
computes correlations across *all* level pairs, not only adjacent ones,
and it uses those extra connections to identify and flag **redundant**
or **artifactual** factors in the hierarchy. A redundant factor restates
a factor at another level rather than refining it. An artifactual factor
is a by-product of the extraction rather than a construct in the data.
...
in substance the same construct as a factor several levels up. That
would be a sign that the intermediate levels are adding noise rather
than resolution.
...
correlation between every pair of levels, not only neighbors.
...
apart) with `|r| >= 0.5`, strongest first. It is drawn from
...
in substance the same construct. The intermediate levels are just
...
correlations (\|r\| ≥ 0.9 by default) across levels. Suppose m2f2 → m3f2
→ m4f2 → m5f1 all share r \> 0.97. The chain reaches the deepest level,
so its bottom node m5f1, the most specific and best-defined
manifestation, is retained. The others (m2f2, m3f2, m4f2) are flagged as
redundant: they repeat rather than refine the same dimension. A chain
that stops *short* of the deepest level instead keeps its **top** node,
the broadest manifestation (Forbes, 2023).
...
`|r| ≥ 0.9` with the leaf, which means it shares at least 81% of its
variance with the leaf directly. This is the rule Forbes’s own code
uses, and the honest reading of “the same construct”. Two things it is
**not**. It does not require each consecutive pair to correlate (that
adjacent-hop rule is `redundancy_criterion = "adjacent"`), and it does
not screen every ancestor pair against every other. Because correlation
is non-transitive, the star and the adjacent-hop walk can disagree in
deep hierarchies. An ancestor can join on a strong direct link to the
leaf even where its hop to the next chain member is weak. On a shallow
...
their loadings (a loading is the correlation between an item and a
factor), use their scores, and include them in the diagram. Pruning
...
factors entirely. Each retained factor then connects directly to its
single strongest kept ancestor, even when that ancestor is several
levels away. This is the Forbes (2023) pruned-factor diagram, activated
by `drop_pruned = TRUE`.
...
measured by Tucker’s congruence coefficient (φ), which compares two
columns of loadings item by item and returns 1 when they are
proportional:
...
φ ranges from −1 to +1. Values above 0.95 are conventionally read as
...
degrees of freedom. A standardized flag that is always computed the same
way *removes* researcher discretion rather than adding it. The reason is
...
evidence for *you* to weigh, and leaves the drop decision to you. The
evidence is Tucker’s φ for **every** cross-level factor pair in
`x$prune$phi`, plus the structural signals of the next section in
`x$prune$structural`.
...
pattern. But a congruence *this* high travels with a near-perfect score
...
and shares a loading pattern `φ = 0.94` is not quite redundant. A
rotation re-orients the factors within a level without changing how well
they fit. Such a pair is a candidate for re-rotation, worth a second
look rather than a clean new level of resolution. The function
`prune(x, "artifact")` collects exactly these in
`x$prune$near_redundant`. It keeps every cross-level pair that is
**not** itself fully redundant yet is close to redundant on either
signal. Close means within `near_margin` (default `0.1`) *below* the
threshold. The first signal is the pair’s direct score correlation
`|r|`. Off PCA (principal component analysis, which summarizes the items
with weighted sums called components), the pair’s Tucker `φ` is a second
signal, judged against its own threshold. One signal in its window is
enough.
...
announces this). EFA (exploratory factor analysis) models the items as a
small number of shared factors plus item-specific noise. ESEM
(exploratory structural equation modeling) fits that same factor model
in lavaan and adds standard errors. Widen or narrow the band with
`near_margin`. Like the φ table, the band is **report-only**: no factor
is dropped on its basis.
...
- **`split_merge`**: the factor’s primary items were spread across *two
  ...
  the finer solution. This is the split-then-merge anomaly of Forbes
  Fig. 2.
...
which the signals only point you toward. The two thresholds, `min_items`
and `orphan_r`, are arguments to
[`prune()`](https://jmgirard.github.io/ackwards/reference/prune.md).
...
researcher inspection, and no factors are auto-flagged.
...
above a threshold), not only a high score correlation. The
...
- **EFA / ESEM**: φ is required to exceed `0.95` (Lorenzo-Seva & ten
  ...
  `|r|`-only rule too liberal, and the loading-congruence guard makes
  the criterion conservative.
  [`prune()`](https://jmgirard.github.io/ackwards/reference/prune.md)
...
because the redundant chains all have correlations \> 0.97. The flagging
is unambiguous. With your own data you may find borderline cases where
...
For the BFI all thresholds agree: the flagged factors are clearly
...
1.  **Do any factors persist unchanged across multiple levels?**
    ...
    dimensions that survive changes in k. These are exactly the kind of
    stable construct you want to report.
...
identify which levels add the most new information. Use that to guide
your focus in reporting. For where redundancy pruning sits in the full
recommended workflow, see
[`vignette("ackwards-girard")`](https://jmgirard.github.io/ackwards/articles/ackwards-girard.md).
That workflow also gates hierarchy depth on split-half replicability
with
