...
one panel for each criterion group that was requested. The groups are
Scree/PA (if `"pa_pc"` or `"pa_fa"` were requested), MAP (if `"map"`),
VSS (if `"vss"`), and CD RMSE (if `"cd"` and EFAtools was available). PA
is parallel analysis, which compares the eigenvalues of your data with
those of random data of the same shape. When four panels are shown the
...
PA-PC threshold, and PA-FA compares the teal "Observed (FA)" line to the
...
significantly reduces RMSE (default \\\alpha = 0.30\\), and the starred
k is the last retained factor. The curve is shown only over the levels
that were actually computed. The starred k need not be the visible
minimum of the plotted curve.
...
#> ✔ Running parallel analysis (5 iterations, PC + FA)... [61ms]
#> 
#> ℹ Running MAP and VSS...
#> ✔ Running MAP and VSS... [104ms]
#> 
