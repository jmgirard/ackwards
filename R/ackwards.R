...
#'   items have a few ordered categories, such as a 1 to 5 rating. Polychoric
#'   correlations estimate the correlation between the continuous traits
#'   assumed to underlie those ordered responses.
...
#'   parent is positive.
#' * **`keep_scores = FALSE` / `keep_fits = FALSE`** for memory and privacy.
#'   Scores are O(n x Sigmak) and often sensitive, and raw engine fits can be
...
#'   `k_max` of `suggest_k()` means something related but different. It is the
#'   largest number of factors that function *evaluates* when recommending a
#'   depth, not a depth itself. See that function's docs.)
#' @param n_obs Number of observations. On the raw-data FIML path it may
#'   instead be a string selecting which N feeds the fit indices. FIML is full
...
#'     `engine = "efa"`, because [psych::fa()] needs N for chi-square, RMSEA,
#'     and TLI. It is optional for `"pca"` (stored as `NA_integer_` if
#'     omitted). For PCA it is recorded in the result metadata and feeds the
#'     N-based sampling-adequacy checks only. PCA level fit is
...
#'     `engine = "pca"`/`"efa"` (M38). [psych::corFiml()] estimates the
#'     correlation matrix from incomplete rows, so `n_obs` may be `"total"`
#'     or `"complete"`. The default is `"total"`, meaning every row that
...
#'     (Zhang & Savalei, 2020). A string `n_obs` is accepted only on this
#'     path.
#' @param engine Extraction engine: `"pca"` (default), `"efa"`, or `"esem"`.
#'   `"esem"` uses [lavaan::efa()] with rotation-aware SEs and per-level fit
#'   indices. ESEM is exploratory structural equation modeling, and it is
...
#'   correlation matrix via `psych::polychoric()` (requires psych). Polychoric
#'   correlations estimate the correlation between the continuous traits
#'   assumed to underlie ordered responses. For ESEM,
...
#'   Ordinal items have a few ordered categories, such as a 1 to 5 rating.
...
#'   (maximum likelihood with standard errors that tolerate non-normal data).
...
#'   valid, if atypical, continuous WLS or ADF estimator. Ignored for PCA and
#'   EFA engines. The
...
#'     sample size on the PCA and EFA path.
...
#'   every pair of levels, including skip-level correlations. The [prune()]
...
#'   `psych::polychoric()` fails on your data, and its error suggests exactly
#'   this. That failure typically happens when an item has a near-empty
#'   response category, or when items with unequal category counts produce a
...
#'   [prune()], which is the Forbes-extension flagging of redundancy (one
#'   factor adding nothing over another) and of artifacts, piped off the
...
#' For ordinal data (`cor = "polychoric"`, WLSMV) lavaan derives costly sample
#' statistics from the raw data. Those are the thresholds, the polychoric
#' correlation matrix, and the asymptotic weight matrix. They depend only on
#' the data, not on the number of factors. So they are **computed once** at
#' the first level and **reused** for every deeper level, which gives
#' identical solutions for much less work. This
...
#' algebra, so no raw item responses are needed. This is useful when you have
...
#' * **`n_obs`:** required for `"efa"`, because psych needs N for chi-square,
#'   RMSEA, and TLI. It is optional for `"pca"`, where it is stored as `NA` if
#'   omitted and is used only for the N-based sampling-adequacy checks and the
#'   result metadata. PCA computes no N-dependent fit statistics.
#' * **`cor` argument:** ignored, because the basis is already determined by
#'   the matrix you supply. A warning is emitted if you set `cor` explicitly.
...
#' * **`psych::polychoric()` fails**, usually because of a near-empty response
#'   category. Set `correct = 0` or collapse rare categories.
#' * A **level fails to converge**. The hierarchy is truncated to the deepest
#'   level that did converge, and you should not interpret beyond it.
...
#'   unreliable. The loadings and edges then rest on a rank-deficient matrix.
#'   (For EFA the residual-based fallback inflates `TLI` and `RMSEA`, and for
#'   ESEM `CFI` comes back `NA`.) Trim redundant items, or use
#'   `missing = "listwise"`. On the polychoric basis you can instead collapse
#'   sparse categories or set `correct = 0`.
#'
#' **Caution, so interpret carefully.** The solution exists but may be
#' unstable:
#' * A **Heywood case** at a level, meaning a communality `> 1` or a negative
#'   uniqueness. Check that level's loadings make substantive sense, and
