...
An **artifact** factor is one whose loading pattern is more similar to a factor
...
at k = 1 is suspected to be an artifact of the rotation rather than a genuine
new dimension.
...
```{r prune-artifact-nodes, echo = FALSE}
...
if (has_gt) {
  gt::gt(art_nodes) |>
    gt::tab_header(
      title    = "Node-level artifact annotation",
      subtitle = paste0(n_artifact, " of ", nrow(art_nodes), " factors flagged as artifacts")
    ) |>
    gt::cols_label(
      id           = "Factor",
      level        = "Level",
  ...
  knitr::kable(
    ...
    col.names = c("Factor", "Level", "Flagged?", "Reason"),
    caption = paste0(
      "Node-level artifact annotation (", n_artifact, " of ", nrow(art_nodes), " factors flagged)"
    )
  )
}
```

For the BFI, the artifact criterion flags `r n_artifact` factors — no factor at
...
to separate highly correlated factors, the artifact criterion will often flag
...
- `"artifact"`: this factor's *loading pattern* closely resembles a factor from
...
- **EFA / ESEM** — φ is required to exceed `0.95` (Lorenzo-Seva & ten Berge,
  ...
  conservative. `prune()` announces this auto-resolution in the console.
