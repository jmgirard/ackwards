...
# Derived, drift-proof prose helpers (used in inline R below)
...
`r n_redundant` factors are flagged as redundant (`r paste(flagged_ids, collapse = ", ")`):
`r lvl_summary`. This is a striking finding: for this dataset and this k, the
...
```{r prune-artefact-nodes, echo = FALSE}
...
if (has_gt) {
  gt::gt(art_nodes) |>
    gt::tab_header(
      title    = "Node-level artefact annotation",
      subtitle = paste0(n_artefact, " of ", nrow(art_nodes), " factors flagged as artefacts")
    ) |>
    gt::cols_label(
      id           = "Factor",
      level        = "Level",
  ...
  knitr::kable(
    ...
    col.names = c("Factor", "Level", "Flagged?", "Reason"),
    caption = paste0(
      "Node-level artefact annotation (", n_artefact, " of ", nrow(art_nodes), " factors flagged)"
    )
  )
}
```

For the BFI, the artefact criterion flags `r n_artefact` factors — no factor at
any level has a loading pattern more similar to a factor from a non-adjacent
level than to its own-level solution. This is a good result for a well-validated instrument: it
...
Congruence (φ) is not the only fingerprint of an artefactual factor. Forbes
(2023, Fig. 2) describes several *structural* signatures, and `prune =
"artefact"` reports three of them per factor in `x$prune$structural`:
...
- **`few_items`** — the factor is the primary (highest-loading) home for fewer
  ...
  under-identified and often an extraction artefact rather than a replicable
  construct.
...
```{r prune-structural, echo = FALSE}
struct <- x_art$prune$structural
n_struct <- sum(struct$few_items | struct$orphan | struct$split_merge, na.rm = TRUE)

if (has_gt) {
  gt::gt(struct) |>
    gt::tab_header(
      title    = "Structural artefact signals",
      subtitle = paste0(n_struct, " of ", nrow(struct), " factors raise a structural signal")
    ) |>
    gt::cols_label(
      id          = "Factor",
      level       = "Level",
  ...
  knitr::kable(
    struct,
    row.names = FALSE,
    col.names = c("Factor", "Level", "Few items", "Orphan", "Split/merge"),
    caption = paste0("Structural artefact signals (", n_struct, " of ", nrow(struct), " flagged)")
  )
}
```

Like Tucker's φ, these signals are **flag-and-report only** — `ackwards()` never
removes a factor on their basis. Identifying an artefact requires researcher
...
two thresholds, `min_items` and `orphan_r`, are arguments to `ackwards()`.
...
- **PCA** — no φ filter. The `W'RW` score algebra is exact for components, so
  the score correlation `|r|` alone is a sufficient redundancy signal.
- **EFA / ESEM** — φ is required to exceed `0.95` (Lorenzo-Seva & ten Berge,
  2006). Factor scores off the PCA basis are indeterminate, which makes a
  `|r|`-only rule too liberal; the loading-congruence guard makes the criterion
  conservative. `ackwards()` announces this auto-resolution in the console.
