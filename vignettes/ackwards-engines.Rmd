...
teaching point: how much smaller EFA loadings are once measurement error is
partitioned into uniqueness.
...
wide$delta <- wide$loading_efa - wide$loading_pca
...
if (has_gt) {
  gt::gt(wide) |>
    ...
    gt::tab_footnote(
      footnote  = "Factor assignment and sign verified to match between engines for all six items.",
      locations = gt::cells_column_labels(columns = delta)
    )
} else {
  knitr::kable(
...
level transition. The Δ column shows how much the EFA correlation differs from
the PCA correlation for the same parent–child pair — a direct measure of how
much the latent-variable model shifts your inference about the hierarchy.
