...
**`"pairwise"` (default).** Correlations are computed using all available pairs
of observations (`pairwise.complete.obs`). This is the classical approach and
works for PCA, EFA, and ESEM. For the ESEM engine with a continuous estimator
(ML/MLR), lavaan additionally performs listwise deletion internally for model
fitting, so fit statistics (CFI, RMSEA, …) are computed at the complete-case N
while edges use the larger pairwise N. This inconsistency is usually small but
is documented in the result's `$meta`. A warning is emitted whenever incomplete
rows are detected with this option.
...
| Want consistent fit statistics and edges | `"listwise"` |
| ESEM ML/MLR, meaningful missingness, want all rows used in estimation | `"fiml"` |
| Ordinal data + WLSMV | `"pairwise"` or `"listwise"` |
