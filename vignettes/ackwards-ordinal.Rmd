...
Stacking `tidy(what = "loadings")` from each basis lets us read the `k = 5`
Neuroticism-item loadings side by side:

```{r load-compare, echo = FALSE}
loadings <- rbind(
  cbind(basis = "pearson", tidy(x_pearson, what = "loadings")),
  cbind(basis = "polychoric", tidy(x_poly, what = "loadings"))
)
loadings <- loadings[loadings$level == 5 & substr(loadings$item, 1, 1) == "N", ]
loadings <- loadings[order(loadings$factor, loadings$item), ]
knitr::kable(
  loadings[, c("basis", "factor", "item", "loading")],
  digits = 2, row.names = FALSE,
  caption = "Neuroticism-item loadings (k = 5): Pearson vs polychoric"
)
```
...
The same comparison for the primary-parent edges — `tidy(what = "edges",
primary_only = TRUE)` under each basis:

```{r edge-compare, echo = FALSE}
edges <- rbind(
  cbind(basis = "pearson", tidy(x_pearson, what = "edges", primary_only = TRUE)),
  cbind(basis = "polychoric", tidy(x_poly, what = "edges", primary_only = TRUE))
)
edges <- edges[order(edges$to, edges$basis), c("basis", "from", "to", "r")]
knitr::kable(
  edges,
  digits = 2, row.names = FALSE,
