...
**CD agrees with PA-FA** (k = 6, when `EFAtools` is available). Both point to a
sixth factor worth examining. CD resamples from the marginal item distributions
without preserving inter-item correlations, so on datasets with strong structure
it can over-retain; here it lands conservatively at 6, in line with the other
liberal criterion.

**Defensible choice: `k_max = 6`.** This matches the PA-FA and CD recommendations
and sits one level above the MAP/VSS-2 consensus of 5. Setting `k_max = 6` lets
`ackwards()` show whether a sixth level reveals interpretable sub-facets or merely
