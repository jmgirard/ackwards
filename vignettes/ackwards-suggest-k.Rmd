...
```{r sim16, message = FALSE}
sk_sim <- suggest_k(sim16, seed = 42)
print(sk_sim)
```

```{r sim16-range, echo = FALSE}
sim_ks <- c(
  sk_sim$k_parallel_pc, sk_sim$k_parallel_fa, sk_sim$k_map,
  sk_sim$k_vss1, sk_sim$k_vss2, sk_sim$k_cd
)
sim_lo <- min(sim_ks, na.rm = TRUE)
sim_hi <- max(sim_ks, na.rm = TRUE)
sim_consensus <- if (sim_hi == sim_lo) {
  paste0("k = ", sim_lo)
} else {
  paste0("k = ", sim_lo, "–", sim_hi)
}
...
bfi_lo <- min(bfi_ks, na.rm = TRUE)
bfi_hi <- max(bfi_ks, na.rm = TRUE)
```

The criteria collapse onto essentially one answer — the consensus is
`r sim_consensus`, recovering the four factors built into `sim16`. This is what
...
criteria span k = `r bfi_lo`–`r bfi_hi`, and no single value is obviously
...
```{r worked-scalars, include = FALSE}
# Drift-proof prose: every number below comes from the sk object, so the text
# cannot contradict the printed table. This matters because parallel analysis
# does not respond to set.seed() (see the `seed` section above) -- PA values
# can differ across vignette builds.
k_map <- sk$k_map
k_v1 <- sk$k_vss1
k_v2 <- sk$k_vss2
k_pc <- sk$k_parallel_pc
k_fa <- sk$k_parallel_fa
k_cd <- sk$k_cd

pa_order_txt <- if (is.na(k_fa)) {
  "In this run PA-FA is undetermined (no FA eigenvalue beat the random
   threshold), so PA-PC stands alone as the liberal bound."
} else if (k_fa > k_pc) {
  paste0(
    "In this run PA-FA (", k_fa, ") exceeds PA-PC (", k_pc, "). Normally ",
    "PA-PC over-extracts relative to PA-FA, making PA-PC the liberal upper ",
    "bound; an inversion like this one reflects the dataset's eigenvalue ",
    "geometry plus PA's run-to-run simulation noise, not a contradiction."
  )
} else {
  paste0(
    "In this run PA-PC (", k_pc, ") sits at or above PA-FA (", k_fa, "), ",
    "the usual ordering: PA-PC tends to over-extract and serves as the ",
    "liberal upper bound."
  )
}

cd_txt <- if (!is.na(k_cd)) {
  paste0(
    "**CD recommends k = ", k_cd, ".** CD generates comparison data that ",
    "reproduce the observed correlation structure under a k-factor model ",
    "(as well as the items' marginal distributions), so its recommendation ",
    "reflects how many factors are needed to mimic the data's actual ",
    "eigenvalue profile. In simulation it is among the most accurate ",
    "criteria, though it can over-retain on large, highly correlated samples."
  )
} else {
  "**CD is unavailable on this build** (the optional EFAtools package is not
   installed), so the range below rests on the other criteria."
}
```
...
**The conservative criteria.** MAP recommends k = `r k_map`; VSS-1 and VSS-2
peak at k = `r k_v1` and `r k_v2`. Where two of these converge, that value is
a meaningful signal — they err on the side of too few factors, so their answer
is a floor worth taking seriously.

**The liberal criteria.** PA-PC retains k ≤ `r k_pc``r if (!is.na(k_fa)) paste0(" and PA-FA k ≤ ", k_fa) else ""`.
`r pa_order_txt` Read both, but trust PA-FA as the model-consistent criterion
for EFA and ESEM engines.

`r cd_txt`

**Defensible choice: the top of the range (here `r bfi_hi`).** Taking the
