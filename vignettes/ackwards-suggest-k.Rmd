...
Before you can run `ackwards()`, you need a value for `k` — the *maximum depth*
of the hierarchy. This is not the same question as "how many factors does my
data really have?" There is no single true k hidden in the data, and different
...
**k is a maximum depth, not a claim about the true structure.** Setting k = 5
tells `ackwards()` to fit models at every level from 1 to 5 and examine how the
structure evolves across those levels. It does not assert that exactly five
factors exist. In fact, deliberately setting k one or two levels past the
...
ceiling with appropriate skepticism.
...
more trustworthy than any single recommendation.
...
research). Treat its recommendation as an *upper bound*.

**When it is useful.** Best paired with `method = "pca"` in `ackwards()`, since
...
**When it is useful.** The model-consistent criterion for `method = "efa"` or
`method = "esem"` in `ackwards()`: if your analysis assumes a common-factor
...
**Limitation.** VSS criteria work poorly when items have meaningful cross-
loadings (a common situation with personality scales). In those cases, VSS-1 in
particular may underestimate k.
...
dataset and retains the factor whose addition most consistently improves RMSE
across the real and comparison data.
...
- PA-PC and PA-FA mark a checkmark at each k they would retain (up to their
  recommended ceiling) and a dash above it.
- MAP, VSS-1, and VSS-2 show the numeric value at each k; the optimal k is
  starred (`*`).
- CD (if available) marks a checkmark up to its recommendation and stars the
  optimal k.
...
```{r autoplot, fig.width = 7, fig.height = 7}
autoplot(sk)
```

The three panels show the full evidence curve at a glance:

**Scree / Parallel Analysis (top).** The solid blue line is the observed PC
...
**MAP (middle).** Lower is better. The criterion minimizes at the starred k.

**VSS (bottom).** Higher is better. VSS-1 (solid green) and VSS-2 (dashed
...
`k_max` here is a search ceiling for `suggest_k()` and need not equal the `k`
you ultimately pass to `ackwards()`.

## A worked recommendation for the BFI

The BFI has 25 items measuring five personality traits. The criteria do not all
converge here, which is itself informative:
...
**MAP and VSS-2 agree on k = 5**, with VSS-1 slightly more conservative at
k = 4. Two conservative criteria converging is a meaningful signal; the modal
answer from the non-PA criteria is 5.

**PA-FA exceeds PA-PC in this run** (6 vs. 5). Normally PA-PC over-extracts
relative to PA-FA, making PA-PC the liberal upper bound. Here the relationship
inverts — a nuance in this dataset's eigenvalue geometry, not a contradiction.
Read both, but trust PA-FA as the model-consistent criterion for EFA and ESEM
engines.

**CD is a high outlier** (k = 8, when `EFAtools` is available). CD is among the
most accurate criteria in simulation studies, but it resamples from the marginal
distributions of each item independently, without preserving inter-item
correlations. On a large, moderately correlated sample like the BFI
(n = 2,436, 25 correlated items), the comparison data underestimates how much
structure the real correlation matrix contains, so CD over-retains factors. Treat
a large gap between CD and the other criteria as an invitation to look carefully
at the deeper levels — not a directive to extract that many factors
unconditionally.

**Defensible choice: `k = 6`.** This matches the PA-FA recommendation and sits
one level above the MAP/VSS-2 consensus of 5. Setting k = 6 lets `ackwards()`
show whether a sixth level reveals interpretable sub-facets or merely splits the
Big Five into arbitrary micro-factors.

```{r ackwards, eval = FALSE}
x <- ackwards(bfi, k = 6, cor = "polychoric")
autoplot(x)
```

If the k = 6 level looks like overextraction (very thin arrows, factors that
split and immediately re-merge, between-level r ≈ 1), you can interpret down to
k = 5 with confidence that you have not missed structure. If k = 6 reveals
interpretable sub-facets, you have found something worth reporting.
...
https://doi.org/10.1037/met0000578
