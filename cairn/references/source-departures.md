...
| E3 | Goldberg: PCA component scores (default engine matches — see M1) | For **factor** engines (EFA/ESEM), scores are **tenBerge**, not components | DESIGN §9 `scores` row; D-007 | Correlation-preserving + linear (algebra-eligible); trade-off formalized by [[grice2001]] / [[beauducel2024]]; Goldberg's own "virtually identical" for PCA-vs-factor | `depart-supported` |
...
| M1 | Goldberg: display paths ≥ **.30** in figures | `cut_show = 0.3` (default show-cut) | DESIGN §9; `R/ackwards.R:292`; [[goldberg2006]] | — (matches the source) | `match` |
