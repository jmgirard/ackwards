...
- Owner-only post-M55 release tail: 0.1.1 original submission **withdrawn 2026-07-16** (lavaan 0.7-2 broke the ESEM tests; fix merged as PR #66, resubmission tarball prepped from master `43b4ce6` — NEWS + cran-comments in PR #67, win-builder R-devel run pending in owner email); **owner: send the withdrawal email, confirm win-builder green, then `devtools::submit_cran()`**; on acceptance tag `v0.1.1` + update README "on CRAN" phrasing; if CRAN bounces again, plan the next resubmission milestone — added 2026-07-12, updated 2026-07-16 — supersedes the 0.1.0 tail row (0.1.0 submitted; 2026-07-12 reviewer feedback became M55, which absorbed the remote-check steps and superseded the patch-branch-from-tag guidance at its plan gate)
...
- Mechanize the precomputed-vignette staleness guard: an edited `vignettes/*.Rmd.orig` without a re-run of `precompute.R` ships stale output; today only CLAUDE.md prose warns (owner-confirmed wart, design interview) — a hash-manifest check (tools/ script or test) would close it — added 2026-07-17
- Upstream re-oracle watch: psych/lavaan releases can move live-oracle values or break engines (lavaan 0.7 → PR #66); consider a scheduled CI job rerunning the suite against fresh CRAN versions, per the track-CRAN-current posture (DESIGN §2) — added 2026-07-17
