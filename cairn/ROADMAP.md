...
- Intermittent Windows CI crash in parallel testthat workers — untracked until now: `R session crashed with exit code -1073741819` (0xC0000005, access violation) kills a testthat subprocess and fails `windows-latest (release)`. Three occurrences, each in a **different** file — 2026-07-24 master `e66ec1e` (`test-cor-input.R`), 2026-07-27 `5abe6d2` (`test-suggest_k.R`), 2026-07-27 PR #90 (`test-suggest_k.R`) — so the suspect is the parallel-worker setup (`Config/testthat/parallel`), not any one test. Green on re-run every time, which is how it stayed invisible for three days. CRAN runs the same parallel config; its Windows flavours are OK as of 0.1.1 and win-builder R-devel passed 0.2.0. Promote on any Windows failure in a CRAN check or win-builder run, or on a second failure within one CI run. — added 2026-07-27
