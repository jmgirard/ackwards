...
- [low] Near-singular within-level score correlation guard for `beta`/`r2`: `.partialled_edges()` warns and returns `NA` only when `solve()` rejects Phi_s outright; a nearly collinear level (rcond around 1e-15) can pass and yield unstable `beta` with no warning. An rcond guard in keeping with `meta$near_singular` (which covers R, not Phi_s) would close it. From the M89 review, finding 14. — added 2026-09-16
