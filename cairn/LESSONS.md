...
- 2026-07-19 (M67, corrected M68): to check quotations against a shelf PDF, extracted text + a Python substring search is far cheaper than page images, but it has two traps that cost M68 four wrong page anchors and one false claim. (a) The extraction **drops inter-word spaces**, so a multi-word search (`"Big Two"`) silently returns zero hits and reads as a confirmed absence — search single tokens (`BigTwo`) or several spellings before ever asserting "does not appear". (b) Running heads interleave with body text in the stream, so a page number adjacent to a passage is **not** its page — confirm every anchor by rendering that page image. Absence claims are only safe for single tokens (e.g. "varimax" occurs zero times). Also: don't grep the flattened text with wide regex windows like `.{130}` — catastrophic backtracking on one long line hangs; use `str.find` plus slicing.
