...
- After the fix-now edits: `check-prose.R` on the three edited sources exits 0, `--code-unchanged master` OK, `precompute.R` re-run (exit 0) with untouched vignettes and assets reverted, vignette freshness OK, and the regenerated outputs differ from master only in prose, stamps, and re-rendered PNGs.
