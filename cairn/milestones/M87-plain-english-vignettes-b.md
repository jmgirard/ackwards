...
- 2026-09-16: step-7 approval: m087-plain-english-vignettes-b approved for merge (findings 1-5, 7, 8 and the record-lens finding fixed on the branch; finding 6 rejected as logged).
...
- Independent review (three fresh-context lenses, ranked by each reviewer, all findings logged):
  - [O] diff-bug 1, `ackwards-forbes.Rmd.orig` near-redundant band: the rewrite made `|r|` the primary signal with `φ` as an extra check, but `R/prune.R` `.near_redundant_pairs()` flags a pair when either signal is in its window. Verified against the source. Fix now: paragraph rewritten to "either signal ... one signal in its window is enough".
  - [O] 2, `ackwards-girard.Rmd.orig` `suggest_k()` table cell "computed from the item correlation matrix" is false for CD, which resamples the raw data (`R/suggest_k.R`). Verified. Fix now: cell reads "Several retention criteria (consensus range)".
  - [O] 3, girard redundancy gloss defined redundancy per factor at the .9 threshold, which also describes the retained node. Fix now: gloss rewritten as a property of the pair, with `prune()` keeping one and flagging the other.
  - [O] 4, girard polychoric gloss said it estimates the correlation between the ordinal items; it estimates the correlation between the continuous traits assumed to underlie them. Fix now: gloss aligned with the ordinal vignette's wording.
  - [O] 5, NEWS "the results they show did not change" versus glyph, timing, and gt-id churn in the regenerated outputs. Fix now by reverting that churn to master line by line (see the record lens finding below); the NEWS sentence stands.
  - [O] 6, the ESEM gloss "adds standard errors" is thin. Rejected at triage: not wrong, and batch A (M86, merged) uses the same gloss, so a stronger one is a cross-vignette edit outside this milestone. Left for the maintainer to redirect at the gate.
  - [O] 7, forbes "It computes" read as the method rather than the author. Fix now: "Her method computes".
  - [O] 8, forbes2023 "It flags" could attach to `m10f4`. Fix now: "The rule flags".
  - [S] blame-history: no findings. Every chunk line byte-identical to master by its own diff; D-017/D-024/D-032/D-033/D-034 claims preserved; the dod-gate widening matches M86's pattern.
  - [S] prior-review record 1: `ackwards-girard.Rmd` carried ten changed cli timing lines that LESSONS M61/M75 say to revert line by line. Confirmed (the other four outputs had none). Fix now: the five timing pairs, the seven `✔︎` variation-selector lines, and the forbes2023 gt div id reverted to master values. GitHub PR comment probe returned an empty array, so no thread walk.
  - Return floor: no finding shows an acceptance criterion failing; all fix-now items are defects inside the intended rewrite. No status change.
- After the fix-now edits: `check-prose.R` on the three edited sources exits 0, `--code-unchanged master` OK, `precompute.R` re-run (exit 0) with untouched vignettes and assets reverted, vignette freshness OK, and the regenerated outputs differ from master only in prose, stamps, and re-rendered PNGs. Gate re-run at 120cfd3: GATE PASSED (check 0/0/0, coverage 100%, style/lint clean, pkgdown index complete). AC1, AC2, and AC5 evidence therefore holds at the post-fix head.
