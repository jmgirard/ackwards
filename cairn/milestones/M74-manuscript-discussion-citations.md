...
- 2026-07-23: review — AC1–AC6 all PASS on fresh evidence; consistency gate clean (cairn_validate exit 0; R-toolchain vacuous, build package untouched; pkgdown OK). Three-lens independent review + scorer: Finding B (em dashes, 93) actioned/fixed; A (opening over-claim, 75) + D (Goldberg attribution, 42) fixed anyway; C (CRAN, 62) rejected for M74 (mirrors unchanged Package-section claim; owner release-tail task). Re-rendered clean after fixes.
...
Three fresh-context reviewers (ref-based git only): [O] diff-bug, [S] blame-history, [S] prior-PR-comments. Findings passed to an [S] confidence scorer (did not generate them). Threshold: ≥80 actioned, <80 logged.

- **[S] prior-PR-comments:** no findings. Located archived Review evidence (M56 Waller-title/bfi25-n/54-vs-55; M73 F1 duplication/F2 "three decades"; `ruscio2012a.md` RMSR-vs-RMSE roxygen note) and confirmed M74 regresses none — notably the `ruscio2012a` bib title reads "of known factorial structure" (no spurious "a"). GitHub PR-thread probe returned `[]`.
- **[O] diff-bug:** all prose claims verified against D-001/D-002/D-007/D-012/D-014/D-022/D-023 + IP7; all 7 bib entries correct (authors/year/journal/vol/pp/DOI, incl. `Kuhl` not `Kühl`, `{HiTOP Utility Workgroup}`, Crossref-recovered revelle DOI); AC3 citations-only confirmed. One finding (C, below).
- **[S] blame-history:** two findings (A, B, below) — both against commit `1629d26` ("accuracy, rendering, and prose-style pass"). Confirmed the four other verifiable Discussion claims match live D-entries.

**Findings and disposition:**

- **Finding B — em dashes reintroduced (score 93; ACTIONED → FIXED).** The new Discussion prose reintroduced em dashes at five sites (`engines—principal`, `intervals—a statement`, `converge—most often…engine—and`, `extensions—and others…need—`), reverting the document-wide em-dash removal that commit `1629d26` deliberately made; the Discussion had become the only em-dash-bearing prose section. Fixed: all five converted to parentheses/commas matching the accuracy-pass style; the en dash in "Schmid–Leiman" (correct) preserved. Re-rendered clean.
- **Finding A — opening over-claim (score 75; logged; FIXED anyway).** Discussion ¶1 read "a workflow that has until now existed only as bespoke, paper-specific scripts," which contradicts the Statement of Need's `1629d26` reframing that credits `psych::bassAckward()` with the core top-down loop and scopes the gap to the *extensions*. Below threshold, but a real internal-consistency point in the author's own draft and cheap to fix while editing ¶1 for B: rescoped the opening to credit the **psych** core loop and scope M74's contribution to the rest of the workflow, mirroring the Statement of Need.
- **Finding D — "in Goldberg's own words" (score 42; logged; FIXED anyway).** ¶4 attributed "sequential rather than truly hierarchical…" as a verbatim Goldberg quotation; `goldberg2006.md` verifies the substance but only the single word "sequential" is a confirmed verbatim quote. Softened to "as Goldberg himself acknowledged" to drop the verbatim implication.
- **Finding C — "available from CRAN" (score 62; logged; REJECTED for M74).** ¶5's availability statement asserts CRAN availability while 0.1.1 is pending acceptance (ROADMAP candidate). Rejected as an M74 change: it mirrors the pre-existing, unchanged Package-section claim (`manuscript.qmd:198`), and revising CRAN phrasing is the owner's release-tail task (updating both surfaces on acceptance) — hedging only the Discussion would create an internal inconsistency. Surfaced to the author at the merge gate as a verify-at-publication item.
