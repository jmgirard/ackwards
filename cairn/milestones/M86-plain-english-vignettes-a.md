...
- 2026-09-16: review: AC1-AC5 verified with fresh evidence, consistency gate and three-lens review passed, 13 findings logged, four fixed at the gate.
- 2026-09-16: step-7 approval: m086-plain-english-vignettes-a approved for merge
...
- 2026-09-16 gate triage (user selected fix-then-merge): F1 NEWS sentence now names the widened guard in `tools/check-prose.R` and says the claim can be checked against any git reference. F2 engines line 468 now reads "`missing = "fiml"` errors for WLSMV/ULSMV". F3 intro 402 and suggest-k 58 now say "repeated random half-splits". F10 suggest-k 409 reads "nearly one answer" ("essentially" is on the banned list). Re-verification: prose checker exit 0 over the four sources and NEWS.md, code guard exit 0, precompute re-run with churn in untouched vignettes and assets reverted, the three regenerated `.Rmd` differ only in the prose, stamps, gt div ids, and timing lines, `Rscript tools/dod-gate.R` exit 0 (check 0/0/0, coverage 100%). F5, F6, F7 go to the prose-checker candidate row at hygiene. F4, F8, F9, F11, F12 rejected as logged.
