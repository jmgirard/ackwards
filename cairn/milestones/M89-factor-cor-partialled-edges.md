...
- [ ] T6: Add `.partialled_edges(W_a, R, E)` that returns `beta` and per-column `r2`, with `NA` and one cli warning on a failed `solve()`. Wire `beta` into `.tidy_edges()` (`R/tidy.R:186`) for every stored pair and `r2` into `.tidy_variance()` (`R/tidy.R:323`). Tests per AC5: identity on three engines, `lm()` oracle on a hand-built correlated-composite case, singular path.
- [ ] T7: Roxygen for `tidy()` and `summary()`, `NEWS.md`, DESIGN Known-limitations removal, ORACLES rows for the `lm()` live oracle and the varimax identity invariant. Run `devtools::document()` and `Rscript tools/dod-gate.R`.
