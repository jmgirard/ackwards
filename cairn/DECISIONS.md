...
**Superseded by D-034 (2026-07-30).** *(See also the RR02 correction: this entry's Context asserted that oblique "confounds the cross-level signal"; RR02 found the oblique edge is a different but fully interpretable quantity, and that what oblique breaks is the lineage overlay. The out-of-scope decision is superseded; the varimax default is retained on repaired grounds.)*
...
### D-033 (2026-07-30): Pruned-view secondary edges default to cross-branch only; same-lineage skip arcs become opt-in

**Context:** M79 shipped `autoplot(drop_pruned = TRUE, show_secondary = TRUE)` drawing every kept cross-level pair that is not the primary edge — cross-branch second parents *and* same-lineage skip arcs — on the reasoning (D-017/D-032) that a direct skip-level `r` is a distinct, non-transitive fact not implied by the primary path. Forbes (2026-07-30 correspondence) draws only the cross-branch links; her published pruned hierarchy (Forbes 2023, Fig. 6B, p. 9) carries exactly three secondary edges — `E1→J7`, `E2→G7`, `G7→J7` — and zero same-lineage arcs. `show_secondary` shipped in 0.2.0, released 2026-07-27.
**Decision:** `autoplot.ackwards()` gains `secondary_scope = c("cross_branch", "all")`, default `"cross_branch"`. The non-transitivity reasoning is unchanged and is what keeps `"all"` available; what changes is which set a bare `show_secondary = TRUE` draws, on the ground that a publication figure should default to the published convention (GP1). Pre-1.0 and three days after 0.2.0, so no deprecation cycle is run — the change is announced loudly in NEWS and roxygen instead (IP6).
**Consequences:** A 0.2.0 call setting `show_secondary = TRUE` renders fewer edges; `secondary_scope = "all"` restores it exactly. Figure 6B becomes a test-backed fidelity oracle (M84 AC2/AC3) — the first oracle in the suite read from a printed figure rather than computed by her reference implementation.
_Source: M84; Forbes (2023) Fig. 6B (p. 9); Forbes correspondence 2026-07-30._

### D-034 (2026-07-30; supersedes D-002): Varimax stays the default; oblique moves from out-of-scope to a gated, documented option

**Context:** D-002 (2026-06) made varimax the only supported rotation and put oblique out of scope, on the recorded ground that oblique "confounds the cross-level signal that is the method's point" — sourced to DESIGN §9's claim that only orthogonal rotation produces interpretable between-level factor score correlations (Goldberg 2006; Kim & Eaton 2015). Two things broke that. (a) Forbes, the method's author, uses oblique rotations in her applied work (2026-07-30 correspondence) and her paper frames the method as supporting orthogonal *or* oblique rotation (fn. 1), with her reference implementation equipped for both. (b) RB02/RR02 audited the claim against page images: it originates with Kim & Eaton (2015, p. 1067), who state it bare and misattribute it to Goldberg (2006), who at p. 356 offered orthogonality as a *preference* justified by regression parsimony and marker separation. RR02's Q1 established the substantive position instead: with `Φ_s` the within-level score correlation, the edge satisfies `E = Φ_s B`, so under varimax (Φ_s = I) the marginal correlation *equals* the Φ-partialled unique contribution, and under oblique the two come apart. The oblique edge stays a fully interpretable total correlation; what oblique breaks is the **lineage overlay** (primary parent, split narrative, additive variance partitioning), not interpretability.
**Decision:** Varimax remains the sole **default** for every engine and no current numerical output changes. Oblique is **no longer out of scope**: it becomes a documented non-default option, whose implementation is gated on the Forbes design session settling the marginal-vs-partialled lineage semantics (which quantity drives primary-parent matching, sign anchoring, and the diagram) before any code lands. RR02's BC1–BC8 bind that future milestone and are carried on its ROADMAP candidate row. RR02's rejected shape is adopted as part of this decision: enabling oblique by plumbing alone — passing a rotation argument through to the engines without the accompanying Φ, scoring, variance, and redundancy work — is ruled out, because it would produce silently wrong numbers rather than erroring.
**Consequences:** DESIGN §9's rotation rationale is restated in the package's own voice (the Φ = I argument) with the Kim & Eaton / Goldberg attribution corrected; `cairn/references/goldberg2006.md` loses the same over-attribution and `forbes2023.md` gains its fn. 1 rotation stance. `factor_cor`, currently produced by every engine and read by nothing, becomes the load-bearing new output if oblique ships. IP9 is untouched — the Forbes-reproducing settings stay the default path.
_Source: RB02/RR02 (2026-07-30); Goldberg (2006) p. 356, p. 348; Kim & Eaton (2015) p. 1067; Forbes (2023) p. 2 + fn. 1; Forbes correspondence 2026-07-30; supersedes D-002._

### D-035 (2026-09-16; under D-011 and D-034): GPArotation returns as a guarded Suggests for the oblique option

**Context:** M21 removed `GPArotation` because varimax routes through `stats::varimax`. psych's oblique rotations (`oblimin`, `promax`, `geominQ`) load `GPArotation` at call time, so the oblique option D-034 admits cannot run on the PCA and EFA engines without it. lavaan's `efa()` rotates without it. RR02 BC8 and D-011 permit a guarded Suggests and forbid an Imports entry.
**Decision:** `GPArotation` enters `Suggests` in M90, guarded on every psych oblique path by `rlang::check_installed()`. It never enters `Imports`. The default varimax path never loads it. The dev and CI environments install it, so the oblique branches count toward the coverage gate without an exemption.
**Consequences:** M89 needs no dependency change. The ESEM-only alternative (oblique through lavaan alone) was weighed at the 2026-09-16 plan gate and rejected because Forbes's reference implementation is oblique PCA and EFA, which is the oracle M90 must match.
_Source: /milestone-plan gate 2026-09-16 (M89, M90); RR02 BC8; D-011; D-034._
